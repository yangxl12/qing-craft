...
| 测试       | Node 内置 `assert`                        | 5 份脚本覆盖剖面、触控输入、网格/相机、上釉和装饰/迁移/存储契约，无测试框架 |
...
截至 2026-08-28，`npm run typecheck` 与 `npm test` 均通过。测试脚本不依赖微信运行时，但完整交互、WebGL、导出、相册权限和分享必须在微信开发者工具或真机验证。
...
| `glazeId` / `glazeMethod`              | 当前六种经典釉的选择；新上釉流程固定完整施釉，旧施釉方式字段仅用于历史作品兼容 |
...
| 2    | `glaze`    | 上釉     | 在两行三列中单选 6 种经典釉，器身实时显示完整釉层、积釉、透胎与高光差异      |
...
- 片元着色器实现双方向光、六类经典釉的透明玻璃层/积釉/窑变/粗糙度差异、历史 4 种施釉遮罩、13 路装饰合成（5 层 + 8 印）、语义分区、21 个程序化纹样/边饰变体和固定种子烧后差异；款识由本地离屏 Canvas 生成透明纹理。
...
| 文件                     | 内容与用途                                                                                 |
| ------------------------ | ------------------------------------------------------------------------------------------ |
| `core/catalog.ts`        | 所有内置目录：5 种器形、3 种泥、上釉页 6 种经典釉（另保留 6 个历史釉 ID）、7 道工序及工具/提示文案 |
...
| `tests/glaze.test.cjs`          | 验证六种经典釉顺序、单选托盘、完整釉层材质参数和历史釉色兼容                          |
...
| 上釉          | 只展示 6 种经典釉单选并统一完整施釉；旧 4 种施釉方式只为历史作品保留             | PRD 中至少 12 种釉、浸釉/刷釉/局部留白等自由方式 |
...
| 测试          | 5 个 Node 脚本直接加载生产 TypeScript 函数                                      | 页面自动化、WebGL 真机编译、导出与性能矩阵       |
